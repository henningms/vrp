...
fn can_enforce_required_stops_order() {
    let problem = Problem {
        plan: Plan {
            jobs: vec![create_delivery_job("job1", (1., 0.)), create_delivery_job("job2", (9., 0.))],
            ..create_empty_plan()
        },
        fleet: Fleet {
...
fn can_handle_required_stops_with_time_windows() {
    ...
    let req_activities: Vec<_> =
        tour.stops.iter().flat_map(|stop| stop.activities().iter()).filter(|a| a.activity_type == "required").collect();

    // At least one required stop should be visited
    assert!(!req_activities.is_empty(), "At least one required stop should be present");
...
fn required_stops_work_with_multiple_vehicle_ids() {
    let problem = Problem {
        plan: Plan {
            jobs: vec![create_delivery_job("job1", (5., 0.)), create_delivery_job("job2", (5., 5.))],
            ..create_empty_plan()
        },
        fleet: Fleet {
            vehicles: vec![VehicleType {
    ...
    for tour in &solution.tours {
        let has_required =
            tour.stops.iter().flat_map(|stop| stop.activities().iter()).any(|a| a.activity_type == "required");
