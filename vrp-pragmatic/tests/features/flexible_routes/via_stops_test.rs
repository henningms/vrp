...
fn can_visit_via_stops_in_preferred_order() {
    ...
    if !via_tags.is_empty() {
        ...
        for tag in &via_tags {
            let expected_idx = expected_order.iter().position(|&t| t == tag).expect("Unknown via tag") as i32;
...
fn via_stops_can_be_skipped_when_not_optimal() {
    ...
    assert!(via_count <= 2, "Via stops can be skipped when not optimal");
...
fn via_stops_prefer_on_route_locations() {
    ...
    let problem = Problem {
        plan: Plan {
            jobs: vec![create_delivery_job("job1", (3., 0.)), create_delivery_job("job2", (9., 0.))],
            ..create_empty_plan()
        },
        fleet: Fleet {
            vehicles: vec![VehicleType {
    ...
    println!("Solution has {} stops", tour.stops.len());
}
