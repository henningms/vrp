...
fn can_combine_required_and_via_stops() {
    let problem = Problem {
        plan: Plan {
            jobs: vec![create_delivery_job("job1", (2., 0.)), create_delivery_job("job2", (12., 0.))],
            ..create_empty_plan()
        },
        fleet: Fleet {
    ...
    assert_eq!(req_tags, vec!["req1", "req2"], "All required stops must be present in order");
    ...
    let all_activities: Vec<(&str, String)> = tour
        ...
        .iter()
        .flat_map(|stop| stop.activities().iter())
        .filter_map(|activity| activity.job_tag.as_ref().map(|tag| (activity.activity_type.as_str(), tag.clone())))
        .collect();

    // Find positions of required stops
    let req1_pos = all_activities.iter().position(|(_, tag)| tag == "req1").expect("req1 not found");
    let req2_pos = all_activities.iter().position(|(_, tag)| tag == "req2").expect("req2 not found");

    assert!(req1_pos < req2_pos, "Required stops must maintain order: req1 at {}, req2 at {}", req1_pos, req2_pos);
...
fn required_stops_take_precedence_over_via_stops() {
    ...
    let stop_sequence: Vec<(String, String)> = tour
        ...
        .iter()
        .flat_map(|stop| stop.activities().iter())
        .filter_map(|activity| activity.job_tag.as_ref().map(|tag| (activity.activity_type.clone(), tag.clone())))
        .collect();

    // Find required stops
    let req_first_pos =
        stop_sequence.iter().position(|(_, tag)| tag == "req_first").expect("req_first must be present");
    let req_second_pos =
        stop_sequence.iter().position(|(_, tag)| tag == "req_second").expect("req_second must be present");

    // Required stops must be in order
    assert!(req_first_pos < req_second_pos, "Required stops must maintain strict order");

    // If via stop is present, it shouldn't break required stop order
    if let Some(via_pos) = stop_sequence.iter().position(|(_, tag)| tag == "via_between") {
        println!("Via stop inserted at position {}", via_pos);
        // Via can be anywhere, but shouldn't break required order
        assert!(req_first_pos < req_second_pos, "Via stops cannot break required stop sequence");
    }
}
...
fn complex_mixed_route_with_jobs_required_and_via() {
    ...
    assert_eq!(checkpoint_tags, vec!["checkpoint1", "checkpoint2"], "All checkpoints must be visited in order");
    ...
    println!("Optional waypoints visited: {:?} out of 3 available", via_tags);

    // Verify solution statistics
    assert!(tour.statistic.duration > 0, "Tour should have positive duration");
    assert!(tour.statistic.distance > 0, "Tour should have positive distance");
}
