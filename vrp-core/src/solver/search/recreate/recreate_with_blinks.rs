...
pub enum BlinkJobOrder {
    ...
    TimeWindowEnd,
    /// Puts jobs with the fewest skill-compatible route groups first, breaking ties by earliest
    /// time-window start.
    SkillScarcity,
    /// Puts jobs with the fewest route groups which pass route-level hard constraints first,
    /// breaking ties by earliest time-window start. Unlike skill scarcity, this also sees capacity
    /// and actor time compatibility.
    RouteScarcity,
}
...
impl RecreateWithBlinks {
    ...
    pub fn new_with_defaults(random: Arc<dyn Random>) -> Self {
        Self::new_weighted(false, random)
    }
    ...
    pub fn new_with_job_order(job_order: BlinkJobOrder, random: Arc<dyn Random>) -> Self {
        Self::new_with_job_ordering(job_order, false, random)
    }

    /// Creates a new instance which always uses the specified job order and, optionally, bounded
    /// downstream-leg sampling on long routes (see [`Self::new_sampled_with_defaults`]).
    pub fn new_with_job_ordering(job_order: BlinkJobOrder, sample_legs: bool, random: Arc<dyn Random>) -> Self {
        let selector: Box<dyn JobSelector> = match job_order {
            ...
            BlinkJobOrder::SkillScarcity => Box::<SkillScarcityJobSelector>::default(),
            BlinkJobOrder::RouteScarcity => Box::<RouteScarcityJobSelector>::default(),
        };

        Self::new(vec![(selector, 1)], 0.01, random.clone()).with_leg_sampling(sample_legs, random)
    }

    fn new_weighted(sample_legs: bool, random: Arc<dyn Random>) -> Self {
        Self::new(
            vec![
                // 1. Random (Weight: 4)
        ...
        .with_leg_sampling(sample_legs, random)
    }

    fn with_leg_sampling(mut self, sample_legs: bool, random: Arc<dyn Random>) -> Self {
        if sample_legs {
            self.leg_selection = LegSelection::Stochastic(random);
        }
        self
    }
}
